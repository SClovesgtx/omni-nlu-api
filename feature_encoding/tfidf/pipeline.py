...
def get_tfidf_corpus(workspace_id):
    file = local_path+"/resources/tf_idf_corpus_{workspace_id}.pickle".format(workspace_id=workspace_id)
    tf_idf_corpus = pickle.load(open(file, "rb"))
    return tf_idf_corpus

def get_intents_dictionary(workspace_id):
    file = local_path+"/resources/intents_dictionary-{workspace_id}.json".format(workspace_id=workspace_id)
...
def save_resources(dic_onehot_intents, intent_dictionary,
                  tf_idf_corpus, workspace_id, X_train, y_train,
                  X_test, y_test):

    file = local_path+"/resources/dic_onehot_intents-{workspace_id}.npy".format(workspace_id=workspace_id)
    ...
    with open(local_path+'/resources/X_train_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'wb') as f:
        np.save(f, X_train)
        f.close()

    with open(local_path+'/resources/y_train_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'wb') as f:
        np.save(f, y_train)
        f.close()

    with open(local_path+'/resources/X_test_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'wb') as f:
        np.save(f, X_test)
        f.close()

    with open(local_path+'/resources/y_test_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'wb') as f:
        np.save(f, y_test)
        f.close()
...
def apply_pipe_tfidf_feature_encoding(train_examples, test_examples, index, es):
    ...
    # join all examples
    all_examples = list(map(lambda item: item[0], train_examples)) + list(map(lambda item: item[0], test_examples))
    intents_name = list(map(lambda item: item[1], train_examples)) + list(map(lambda item: item[1], test_examples))
    ...
    dic_onehot_intents = {intent: onehot for intent, onehot in zip(intents_name_set, intents_name_as_onehot)}
    ...
    y_train = np.stack([dic_onehot_intents[intent] for intent in map(lambda x: x[1], train_examples)])

    X_test = np.stack(tf_idf[stard_idx_test:])
    y_test = np.stack([dic_onehot_intents[intent] for intent in map(lambda x: x[1], test_examples)])

    # save resources
    save_resources(dic_onehot_intents=dic_onehot_intents,
