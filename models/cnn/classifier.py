...
def set_model(input_shape, number_of_intent_classes, settings):
    ...
    # input
    model.add(Dense(units=input_shape[0], activation='relu', input_shape = input_shape,
                    name='input_layer'))
    ...
    if settings != None:
        ...
        for layer_defination in settings["intermediate_layers"]:
            model.add(Dense(
                units=layer_defination["number_of_neurons"] if layer_defination.get("number_of_neurons", None) != None else input_shape[0], 
                activation=layer_defination["activation"] if layer_defination.get("activation", None) != None else "relu", 
                name=f'intermediate_layer_{layer_number}'))
            layer_number += 1
        learning_rate = settings["learning_rate"] if settings.get("learning_rate", None) != None else 0.001
        loss = settings["loss"] if settings.get("loss", None) != None else "categorical_crossentropy"
    else:
    ...
    # Output
    model.add(Dense(units=number_of_intent_classes,
