...
MAPPINGS_PROPERTIES = {
    "properties": {
        ...
        "examples": {
            "properties": {
                "text": {"type": "text", "analyzer": "examples_intent_analyzer"}
            }
        },
        "for_train": {
            "properties": {
...
DEFAULT_RECIPE = [
    {
        "model": "cnn",
        "settings": {
            "intermediate_layers": [{"activation": "relu"}],
...
def get_nlp_model(es, workspace_id):
    ...
    if not index.index_exist(es):
        return Response("Model %s do not exist!" % workspace_id, status=404), False
    return index, True
...
def intent_exist(es, index, intent):
    res = es.search(index=index, body={"query": {"term": {"intent": intent}}})
    if res["hits"]["hits"]:
...
def entity_exist(es, index, entity):
    res = es.search(index=index, body={"query": {"term": {"entity": entity}}})
    if res["hits"]["hits"]:
...
class NLPmodelIndex:
    def __init__(self, es, workspace_id, customer_id=None, recipe=None):
        ...
        if not index_exist(es, self.index_alias):
            self.index_name = "nlp_model-{workspace_id}-{dt_string}".format(
                workspace_id=workspace_id, dt_string=dt_string
            )
            ...
            self.recipe = recipe
            self.mapping = {"settings": SETTINGS, "mappings": MAPPINGS_PROPERTIES}
    ...
    def create_recipe(self, es):
        ...
        try:
            ...
            res = es.create(index=self.index_name, id=self.workspace_id, body=doc)

            return (
                False,
                res,
            )  # false to indicate that recipe did not existed and now it was created
        except ConflictError:
            return (
                True,
                "Recipe already exist!",
            )  # true to indicate that document already exist
    ...
    def update(self, es):
        ...
        self.index_name = "nlp_model-{workspace_id}-{dt_string}".format(
            workspace_id=self.workspace_id, dt_string=dt_string
        )
        self.recipe = res["_source"]["recipe"]
        set_bm25_parameters(self.recipe)
        self.mapping = {"settings": SETTINGS, "mappings": MAPPINGS_PROPERTIES}
