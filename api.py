...
from utils.split_data import update_intents_with_messing_examples, train_test_split, get_train_test
from feature_encoding.tfidf.pipeline import apply_pipe_tfidf_feature_encoding, get_tfidf_corpus, get_sentece_features, get_intents_dictionary
...
def nlp_models_crud():
    try:
        if request.method == "GET":
            ...
            res = es.update(index=index.index_name,
        ...
        print(sys.exc_info())
        return Response('{"response":"%s"}'%trace_back_error,
...
def intent_crud():
    try:
        if request.method == "POST":
            ...
            if intent_id:
                try:
                    res = es.get(index=index.index_name,
                                id=intent_id)
                    return {"result": res}
                except NotFoundError:
                    return Response("Intent with id %s was not found!"%intent_id, status=404)
            intent_name = request.args.get('intent_name')
            if intent_name:
                res = es.search(index=index.index_name, body={
                                                          "query": {
                                                            "term": {
                                                              "intent": intent_name
                                                            }
                ...
                if res["hits"]["hits"]:
                    ...
                    return Response("Intent with name %s was not found!"%intent_name, status=404)
            return Response("Expect to recive workspace_id and intent_name or intent_id!", status=400)
            ...
            if intent_id:
                try:
                    res = es.delete(index=index.index_name,
                                id=intent_id)
                    return res
                except NotFoundError:
                    return Response("Intent with id %s was not found!"%intent_id, status=404)
        ...
        if request.method == "GET":
            ...
            if entity_id:
                try:
                    ...
                    return Response("Entity with id %s was not found!"%entity_id, status=404)
            ...
            if entity_name:
                ...
                if res["hits"]["hits"]:
                    ...
                    return Response("Entity with name %s was not found!"%entity_name, status=404)
            return Response("Expect to recive workspace_id and entity_name or entity_id!", status=400)
            ...
            if entity_name:
                ...
                if res["hits"]["hits"]:
                    ...
                    return Response("Entity_id with name %s was not found!"%entity_name, status=404)
            return Response("Expect to recive workspace_id and entity_name or entity_id!", status=400)
...
def read_td_idf_train_test_data(workspace_id):
    ...
    with open(path+'/X_train_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'rb') as f:
        X_train = np.load(f)

    with open(path+'/y_train_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'rb') as f:
        y_train = np.load(f)

    with open(path+'/X_test_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'rb') as f:
        X_test = np.load(f)

    with open(path+'/y_test_tfidf_{workspace_id}.npy'.format(workspace_id=workspace_id), 'rb') as f:
        y_test = np.load(f)
...
def nlp_model_classify():
    if request.method == "POST":
        ...
        if not sentence:
            return Response("Expect to recive a sentence for classification",
                            status=400)
        workspace_id = request.args.get('workspace_id')
        index, exist = get_nlp_model(es, workspace_id=workspace_id)
        if not exist:
        ...
        if fuzzy_match_threshold:
            try:
                ...
                if fuzzy_match_threshold < 0 or fuzzy_match_threshold > 100:
                    return Response("The fuzzy_match_threshold must be between 0 and 100.",
                                    status=400)
            except:
                return Response("The fuzzy_match_threshold must be a float.",
                                status=400)
        else:
        ...
        sentence_tokens = get_tokens(sentence=sentence,
        ...
        if bm25_result != False:
            ...
            ensemble_result = (logit_result + cnn_result + svm_result)/(accuracies["logit_accuracy"] + accuracies["svm_accuracy"] + accuracies["cnn_accuracy"])
        ...
        result = dict()
        result["intents"] = [{"intent": intents_dictionary[str(i)], "confidence": ensemble_result[0][i]}
