...
def instantiate(input_shape, number_of_intent_classes, settings):
...
def set_cnn(X_train, y_train, X_test, y_test, workspace_id, settings=None):
    input_shape = (X_train.shape[1],)
    number_of_intent_classes = y_train.shape[1]

    if settings != None:
        model = instantiate(input_shape, number_of_intent_classes, settings)
        history = model.fit(
            X_train,
            y_train,
