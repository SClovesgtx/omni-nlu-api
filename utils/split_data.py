...
def keep_token(token):
    return (token.is_alpha and
            not (token.is_space or token.is_punct))
...
def complete_messing_examples(intent):
    ...
    if messing_examples < 0 and messing_examples > -5:
        ...
        artificial_examples = [{"constructed":create_new_sentence(sentence["text"]),
                                "original": sentence["text"]} for sentence in examples_]
        ...
        artificial_examples = [{"constructed":create_new_sentence(sentence["text"]), "original": sentence["text"]} for sentence in examples_]
        intent["artificial_examples"] = artificial_examples
    return intent
...
def train_test_split(index, es, percentage=0.3):
    ...
    for intent in intents:
        ...
        if quantity_of_examples >= 8:
            ...
            examples_indices_for_test = set(random.Random(42).sample(population=all_indices, k=qt))
            examples_indices_for_train = all_indices - examples_indices_for_test
            intent["for_train"] = list(map(intent["examples"].__getitem__, examples_indices_for_train))
            intent["for_test"] = list(map(intent["examples"].__getitem__, examples_indices_for_test))
            ...
            examples_indices_for_train = all_indices - examples_indices_for_test
            intent["for_train"] = list(map(intent["examples"].__getitem__, examples_indices_for_train))
            intent["for_test"] = list(map(intent["examples"].__getitem__, examples_indices_for_test)) + [{"text": item["constructed"]} for item in intent["artificial_examples"]]
        elif quantity_of_examples < 5:
            intent["for_train"] = intent["examples"]
            intent["for_test"] = [{"text": item["constructed"]} for item in intent["artificial_examples"]]
